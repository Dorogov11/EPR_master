...
class QuantumAnalysis(object):
    ...
    def analyze_variation(self,
        ...
        if modes is not None:
            # NOTE patch below
            _only_local_modes = list(range(len(modes)))
            PJ = PJ[_only_local_modes, :]
            SJ = SJ[_only_local_modes, :]
            Om = Om[_only_local_modes, :][:, _only_local_modes]
            PHI_zpf = PHI_zpf[_only_local_modes, :]
            PJ_cap = PJ_cap[:, junctions]
        # Analytic 4-th order
